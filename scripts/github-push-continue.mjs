#!/usr/bin/env node
/**
 * GitHub Push Continuation Script - uploads files not yet in the repo.
 * Checks each file against the repo and only uploads missing/changed ones.
 */
...
const START_FROM = parseInt(process.argv[3] || '0', 10);
...
async function run() {
  ...
  const batch = files.slice(START_FROM);
  
  console.log(`\n📦 Gawa → GitHub (continuing from file ${START_FROM + 1})`);
  console.log(`   Uploading files ${START_FROM + 1}–${files.length} (${batch.length} files)\n`);
  ...
  for (const fp of batch) {
    ...
    try {
      ...
      process.stdout.write(`\r   ✓ ${START_FROM + done}/${files.length} — ${relPath.slice(0, 60).padEnd(60)}`);
    } catch (e) {
      failed++;
